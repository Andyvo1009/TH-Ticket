...
const eventApi = {
  ...
  getProvinces: async () => {
    try {
      const response = await fetch('/api/provinces/?depth=1', {
        method: 'GET'
      });
      const data = await response.json();
  ...
  getDistricts: async (provinceCode: number) => {
    try {
      const response = await fetch(`/api/provinces/p/${provinceCode}?depth=2`, {
        method: 'GET'
      });
      const data = await response.json();
  ...
  getWards: async (districtCode: number) => {
    try {
      const response = await fetch(`/api/provinces/d/${districtCode}?depth=2`, {
        method: 'GET'
      });
      const data = await response.json();
